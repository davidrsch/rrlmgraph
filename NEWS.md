...
* `plot.rrlm_graph()`: replaced the static `igraph::plot.igraph()` backend
  ...
  any browser with full pan-and-zoom support.  A new `layout` parameter selects
  the Graphviz engine (`"dot"` hierarchical, `"neato"`, `"fdp"`, `"sfdp"`,
  `"circo"`); a new `file` parameter saves to `.html` (via **htmlwidgets**) or
  to `.png`/`.pdf`/`.svg` (via **webshot2**).
