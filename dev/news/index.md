...
- `precompute-vignettes.yml`: Added R CMD check gate step (runs
  `rcmdcheck::rcmdcheck(args=c("--no-manual","--ignore-vignettes"), error_on="error")`)
  before committing precomputed artefacts; commit step now has
  `if: success()` to prevent committing on check failure
  ([\#119](https://github.com/davidrsch/rrlmgraph/issues/119),
  [\#126](https://github.com/davidrsch/rrlmgraph/issues/126)).
- All workflow files: Re-pinned `r-lib/actions/*` from floating `v2` tag
  to full 40-character commit SHA
  `6f6e5bc62fba3a704f74e7ad7ef7676c5c6a2590` (v2.11.4, 2025-10-08) to
  eliminate supply-chain risk (OWASP A06)
  ([\#134](https://github.com/davidrsch/rrlmgraph/issues/134)).
- `precompute-vignettes.yml`: Precomputed RDS files
  (`vignettes/precomputed/llm_chat_outputs.rds`,
  `vignettes/precomputed/benchmark_summary.rds`) are regenerated by CI
  after each benchmark run. GitHub Actions `workflow_run` triggers are
  scoped to a single repository, so after completing a benchmark run in
  `rrlmgraph-bench`, trigger this workflow manually from the Actions tab
  ([\#135](https://github.com/davidrsch/rrlmgraph/issues/135)).
- All workflow files: Updated `r-lib/actions/*` pins from stale Dec-2021
  SHA (`6f6e5bc`) to floating `v2` tag
  ([\#122](https://github.com/davidrsch/rrlmgraph/issues/122)). Added
  `.github/dependabot.yml` to auto-update GitHub Actions monthly.
- `test-coverage.yaml`: Moved `GITHUB_PAT` from job-level `env:` to the
  specific “Test coverage” step that needs it
  ([\#123](https://github.com/davidrsch/rrlmgraph/issues/123)).
- `pkgdown.yaml`: Replaced `| first // empty | . == "success"` jq filter
  with `| if length == 0 then false else all(. == "success") end` so
  that ALL R-CMD-check matrix legs must pass before docs deploy
  ([\#125](https://github.com/davidrsch/rrlmgraph/issues/125)).

#### Bug fixes (adversarial audit)

- `graph_traverse.R`
  [`query_context()`](https://davidrsch.github.io/rrlmgraph/dev/reference/query_context.md):
  BFS traversal now uses `mode = "out"` instead of `mode = "all"` in
  both
  [`igraph::neighbors()`](https://r.igraph.org/reference/neighbors.html)
  calls (initial frontier and inner-loop expansion). Directed graphs
  require `mode = "out"` for forward traversal (following dependency
  edges from callers to callees), consistent with the forward-directed
  BFS described in arXiv:2512.24601
  ([\#133](https://github.com/davidrsch/rrlmgraph/issues/133)).
- `task_trace.R`: Changed timestamp parse format from
  `"%Y-%m-%dT%H:%M:%SZ"` to `"%Y-%m-%dT%H:%M:%OSZ"` so that
  millisecond-precision ISO 8601 strings emitted by TypeScript
  (e.g. `"2025-06-19T14:32:07.123Z"`) are parsed correctly
  ([\#120](https://github.com/davidrsch/rrlmgraph/issues/120)).
- `node_embed.R` `.fit_tfidf()`: Added explicit NULL guard after
  accessing `tfidf$.__enclos_env__$private$idf` so that a changed
  text2vec internal API produces a clear error instead of silent `NULL`
  propagation
  ([\#121](https://github.com/davidrsch/rrlmgraph/issues/121)).
- `sqlite_export.R`: Changed `task_traces` schema default for `polarity`
  from `1.0` to `0.0`, aligning with the application cold-start default
  ([\#127](https://github.com/davidrsch/rrlmgraph/issues/127)).
- `relevance.R`: Corrected stale file-header comment from
  `R/graph_traverse.R` to `R/relevance.R`
  ([\#128](https://github.com/davidrsch/rrlmgraph/issues/128)).
...
- Future version bumps should be made as standalone commits using
  ...
  rather than buried inside refactoring commits
  ([\#124](https://github.com/davidrsch/rrlmgraph/issues/124)).
